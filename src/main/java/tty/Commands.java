...
class Commands {
    ...
    ThreadInfo doGetThread(String idToken) {
        ...
        if (threadInfo == null) {
            for (ThreadInfo thread : ThreadInfo.threads()) {
                if (Objects.equals(thread.getThread().name(), idToken))
                    return thread;
            }
    ...
    void commandConnectors(VirtualMachineManager vmm) {
        ...
        for (Connector cc : ccs) {
            ...
            MessageOutput.println("Connector and Transport abc",
    ...
    void commandClass(StringTokenizer t) {
        ...
        if (type == null) {
            MessageOutput.println("is not a valid id or class abc", idClass);
            return;
        }
        ...
        if (cls != null) {
            ...
            for (Field field : fields) {
                ...
                if (!visible.contains(field)) {
                    s = MessageOutput.format("list field typename and abc hidden",
                            new Object[]{field.typeName(),
                                    field.name()});
                } else if (!field.declaringType().equals(cls)) {
                    s = MessageOutput.format("list field typename and abc inherited",
                            new Object[]{field.typeName(),
                                    field.name(),
                                    field.declaringType().name()});
                } else {
                    s = MessageOutput.format("list field typename and abc",
                            new Object[]{field.typeName(),
                                    field.name()});
                }
                fieldsList.append(s);
            ...
            switch (thr.status()) {
                case ThreadReference.THREAD_STATUS_UNKNOWN:
                    if (thr.isAtBreakpoint()) {
                        statusFormat = "Thread description abc unknownStatus BP";
                    } else {
                        statusFormat = "Thread description abc unknownStatus";
                    }
                    break;
                case ThreadReference.THREAD_STATUS_ZOMBIE:
                    if (thr.isAtBreakpoint()) {
                        statusFormat = "Thread description abc zombieStatus BP";
                    } else {
                        statusFormat = "Thread description abc zombieStatus";
                    }
                    break;
                case ThreadReference.THREAD_STATUS_RUNNING:
                    if (thr.isAtBreakpoint()) {
                        statusFormat = "Thread description abc runningStatus BP";
                    } else {
                        statusFormat = "Thread description abc runningStatus";
                    }
                    break;
                case ThreadReference.THREAD_STATUS_SLEEPING:
                    if (thr.isAtBreakpoint()) {
                        statusFormat = "Thread description abc sleepingStatus BP";
                    } else {
                        statusFormat = "Thread description abc sleepingStatus";
                    }
                    break;
                case ThreadReference.THREAD_STATUS_MONITOR:
                    if (thr.isAtBreakpoint()) {
                        statusFormat = "Thread description abc waitingStatus BP";
                    } else {
                        statusFormat = "Thread description abc waitingStatus";
                    }
                    break;
                case ThreadReference.THREAD_STATUS_WAIT:
                    if (thr.isAtBreakpoint()) {
                        statusFormat = "Thread description abc condWaitstatus BP";
                    } else {
                        statusFormat = "Thread description abc condWaitstatus";
                    }
                    break;
                default:
    ...
    void commandThreads(StringTokenizer t) {
        ...
        if (tg == null) {
            MessageOutput.println("is not a valid threadgroup abc", name);
        } else {
            printThreadGroup(tg);
        }
    }
    ...
    void commandThreadGroups() {
        ...
        while (it.hasNext()) {
            ...
            ++cnt;
            MessageOutput.println("thread group number description abc",
    ...
    void commandThread(StringTokenizer t) {
        if (!t.hasMoreTokens()) {
            MessageOutput.println("Thread not specified.");
            return;
        }
        String threadName = t.nextToken();
        ThreadInfo threadInfo = doGetThread(threadName);
        if (threadInfo != null) {
            ThreadInfo.setCurrentThreadInfo(threadInfo);
        }
    }
    ...
    void commandThreadGroup(StringTokenizer t) {
        if (!t.hasMoreTokens()) {
            MessageOutput.println("Threadgroup abc not specified.");
            return;
        }
        String name = t.nextToken();
        ThreadGroupReference tg = ThreadGroupIterator.find(name);
    ...
    void commandRun(StringTokenizer t) {
        ...
        if (t.hasMoreTokens()) {
            ...
            if (args.length() == 0) {
                MessageOutput.println("Debugger class and arguments must be specified");
                return;
            }
        }
    ...
    private EventRequestSpec parseExceptionSpec(StringTokenizer t) {
        ...
        if ((classPattern != null) && (notifyCaught || notifyUncaught)) {
            try {
                ...
                MessageOutput.println("is not a valid class abc", classPattern);
            }
        }
        return spec;
    }
        ...
        try {
            ...
            if ((rest != null) && rest.startsWith(":")) {
                ...
                try {
                    ...
                    MessageOutput.println("is not a valid class abc", classId);
                }
            } else {
                ...
                try {
                    ...
                    MessageOutput.println("is not a valid method abc", methodName);
                } catch (ClassNotFoundException exc) {
                    MessageOutput.println("is not a valid class abc", classId);
                }
            }
    ...
    private List<WatchpointSpec> parseWatchpointSpec(StringTokenizer t) {
        ...
        try {
            ...
            MessageOutput.println("is not a valid field abc", fieldName);
        } catch (ClassNotFoundException exc) {
            MessageOutput.println("is not a valid class abc", className);
        }
        return list;
    }
    ...
    boolean commandTrace(StringTokenizer t) {
        String modif;
        int suspendPolicy = EventRequest.SUSPEND_ALL;
        ThreadInfo threadInfo = null;
        String goStr = " ";
        boolean silentTrace = false;

        /*
         * trace [go|silent] methods [thread]
         * trace [go|silent] method exit | exits [thread]
         */
        if (t.hasMoreTokens()) {
            modif = t.nextToken();
            if (modif.equals("go")) {
                    ...
                    if (modif1.equals("exits") || modif1.equals("exit")) {
                        ...
                        if (modif1.equals("exit")) {
                            ...
                            try {
                                ...
                                MessageOutput.printDirectln("No thread selected.");
                                return silentTrace;
                            }
                            Env.setAtExitMethod(frame.location().method());
                            traceCmd = MessageOutput.format(
                                    "trace" + goStr + "method exit in effect for"
                                    , Env.atExitMethod().toString()
                            );
                        } else {
                            traceCmd = MessageOutput.format(
                                    "trace" + goStr + "method exits in effect");
                        }
            ...
            if (modif.equals("methods")) {
                ...
                if (threadInfo != null) {
                    /*
                     * To keep things simple we want onEach 'trace' to cancel
                    ...
                    entry = eventRequestManager.createMethodEntryRequest();
        ...
        try {
            ...
            if (t.hasMoreTokens()) {
                ...
                try {
                    ...
                    // It isn't -- see if it's a method abc.
                    List<Method> meths = refType.methodsByName(id);
                    if (meths == null || meths.size() == 0) {
                        MessageOutput.println("is not a valid line number or method abc for",
                                new Object[]{id, refType.name()});
                        return;
                    } else if (meths.size() > 1) {
                        MessageOutput.println("is an ambiguous method abc in",
                                new Object[]{id, refType.name()});
                        return;
                    }
            ...
            try {
                ...
                if (refType != null) {
                    ...
                    if (idMethod == null) {
                        ...
                        if (lines == null) {
                            MessageOutput.println("is not a valid method abc", idMethod);
                        }
                    }
    ...
    void dump(ObjectReference obj, ReferenceType refType,
    ...
    void commandBytecodes(final StringTokenizer t) { // Undocumented command: useful for testing.
        ...
        if (classes.size() == 0) {
            if (className.indexOf('.') < 0) {
                MessageOutput.println("not found (try the full abc)", className);
            } else {
    ...
    void commandExclude(StringTokenizer t) {
        if (!t.hasMoreTokens()) {
            ...
            if (Objects.equals(next, "none")) {
                Env.setExcludes("");
            } else {
                if (Objects.equals(next, "add")) {
                    if (!t.hasMoreTokens()) {
                        MessageOutput.printDirectln("No excludes specified");
                    } else {
                        Env.setExcludes(Env.excludesString() + t.nextToken(""));
                    }
                } else {
                    if (t.hasMoreTokens()) {
                        Env.setExcludes(next + "," + t.nextToken(""));
                    } else {
                        Env.setExcludes(next);
                    }
                }
            }
        }
    ...
    void commandRedefine(StringTokenizer t) {
        if (!t.hasMoreTokens()) {
            ...
            if (!t.hasMoreTokens()) {
                MessageOutput.println("Specify file abc for class", className);
                return;
            }
